...
import { EllipsisHorizontalIcon, EyeIcon, PencilIcon, CheckCircleIcon } from '@heroicons/vue/24/outline';
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-200">
                    <tr v-for="payment in payments" :key="payment.id" @click="handleRowClick($event, payment)" :class="['hover:bg-gray-50 cursor-pointer', selectedIds.includes(payment.id) && 'bg-blue-50']">
                        ...
                        <td class="px-4 py-3 text-center"><span :class="['px-2.5 py-0.5 rounded-full text-xs font-medium', payment.status === 'PAID' ? 'bg-green-100 text-green-800' : 'bg-amber-100 text-amber-800']">{{ payment.status === 'PAID' ? 'Plaćeno' : 'Planirano' }}</span></td>
    ...
    <Teleport to="body">
        <div v-if="openMenuId !== null && currentPayment">
            ...
            <div class="fixed w-48 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-50" :style="{ top: menuPosition.top + 'px', left: menuPosition.left + 'px' }">
                ...
                <template v-if="currentPayment.status === 'PLANNED' && page.props.auth.user?.canModify">
                    ...
                    <hr class="my-1" />
                    <button @click="handleMarkAsPaid(currentPayment.id)" class="w-full flex items-center gap-2 px-4 py-2 text-sm text-green-600 hover:bg-gray-50"><CheckCircleIcon class="h-4 w-4" /> Označi kao plaćeno</button>
                </template>
            </div>
        </div>
    </Teleport>
    ...
    <Modal :show="showViewModal" title="Detalji plaćanja" @close="showViewModal = false">
        <div v-if="viewingPayment" class="space-y-4">
            <div class="grid grid-cols-2 gap-4">
                ...
                <div><p class="text-sm font-medium text-gray-500">Status</p><span :class="['px-2.5 py-0.5 rounded-full text-xs font-medium', viewingPayment.status === 'PAID' ? 'bg-green-100 text-green-800' : 'bg-amber-100 text-amber-800']">{{ viewingPayment.status === 'PAID' ? 'Plaćeno' : 'Planirano' }}</span></div>
            ...
            <div class="flex justify-end gap-3 pt-4">
                ...
                <button v-if="viewingPayment.status === 'PLANNED' && page.props.auth.user?.canModify" @click="handleEdit(viewingPayment)" class="px-4 py-2 text-sm font-medium text-white bg-blue-500 rounded-lg hover:bg-blue-600">Uredi</button>
            </div>
        </div>
    </Modal>
</template>

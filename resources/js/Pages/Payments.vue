...
const activeFiltersCount = computed(() => {
    return [currencyFilter.value, supplierFilter.value].filter(f => f !== 'all').length;
});
...
<template>
    <MainLayout>
        <Header title="Plaćeno" />
        <div class="p-6 space-y-6">
            <div class="flex flex-col gap-4">
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div class="relative w-full sm:w-96">
                        <MagnifyingGlassIcon class="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" />
                        <input v-model="searchQuery" @keyup.enter="applyFilters" type="search" placeholder="Pretraži po dobavljaču, br. fakture..." class="w-full pl-9 pr-4 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" />
                    </div>
            ...
            <div class="flex items-center justify-between">
                <CurrencySummary :totalKM="summary.totalKM" :totalEUR="summary.totalEUR" :totalUSD="summary.totalUSD" label="Ukupno plaćeno:" />
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-sm font-medium bg-green-100 text-green-800">{{ summary.count }} plaćanja</span>
            </div>
            ...
            <div class="rounded-lg border border-gray-200 bg-white">
                    ...
                    <table class="w-full">
                        <thead>
                            <tr class="border-b border-gray-200 bg-gray-50">
                                ...
                                <th class="px-4 py-3 text-left text-xs font-semibold uppercase text-gray-500">Datum plaćanja</th>
                ...
                <div v-if="payments.length === 0" class="py-12 text-center text-gray-500 text-sm">Nema plaćenih faktura</div>
            </div>
        </div>
        ...
        <Modal :show="showViewModal" title="Detalji plaćanja" @close="showViewModal = false">
            <div v-if="viewingPayment" class="space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div><p class="text-sm font-medium text-gray-500">Iznos</p><p :class="['text-sm font-semibold', getCurrencyColor(viewingPayment.currency)]">{{ formatCurrency(viewingPayment.amount, viewingPayment.currency) }}</p></div>
                    <div><p class="text-sm font-medium text-gray-500">Status</p><span class="px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">Plaćeno</span></div>
                    <div><p class="text-sm font-medium text-gray-500">Datum plaćanja</p><p class="text-sm text-gray-900 font-medium">{{ formatDate(viewingPayment.paid_date) }}</p></div>
                    <div><p class="text-sm font-medium text-gray-500">Planirani datum</p><p class="text-sm text-gray-500">{{ formatDate(viewingPayment.planned_date) }}</p></div>
                </div>
                ...
                <div class="flex justify-between gap-3 pt-4">
                    ...
                    <div class="flex gap-2">
                        <button v-if="page.props.auth.user?.canModify" @click="handleMarkAsUnpaid(viewingPayment.id)" class="px-4 py-2 text-sm font-medium text-amber-700 bg-amber-100 rounded-lg hover:bg-amber-200">Označi kao neplaćeno</button>
